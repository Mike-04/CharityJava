...
public class LoggedInController implements IObserver {
    ...
    public void setStuff(IAppService service, User user) {
    ...
    List<CazCaritabil> cazuri = service.getAllCazuri();
...
public void populateDonatorTable() {
    ...
    List<Donator> donatori = service.getDonators(search);
        ...
        if (selectedCase != null && selectedDonor != null) {
            try {
                ...
                CompletableFuture.runAsync(() -> {
                    service.addDonation(selectedDonor, selectedCase, damount);
                });
                showMessage("Donation made by " + selectedDonor.getNume() + " to " + selectedCase.getNume());
    ...
    private void logout() {
        try {
            service.logout(user.getUsername(), this);
        } catch (Exception e) {
            showMessage("Logout failed: " + e.getMessage());
        }
